...
class _PrayerTimesScreenState extends ConsumerState<PrayerTimesScreen> {
        ...
        children: [
          ...
          if (!_notificationsEnabled) ...[
            ...
            Container(
              ...
              child: const Row(
                children: [
                  Icon(Icons.notifications_off, color: Colors.orange, size: 18),
                  SizedBox(width: 8),
                  Expanded(
                    child: Text(
      ...
      child: Column(
        children: [
          const Text('الصلاة القادمة',
              style: TextStyle(fontFamily: 'NotoNaskhArabic', color: Colors.white70, fontSize: 14)),
          const SizedBox(height: 8),
          Text(ArabicUtils.prayerName(data.nextPrayerName),
              style: const TextStyle(fontFamily: 'NotoNaskhArabic', color: Colors.white, fontSize: 32, fontWeight: FontWeight.w700)),
          const SizedBox(height: 16),
          Text(ArabicUtils.formatPrayerCountdown(_countdown),
              style: AppTypography.prayerTime.copyWith(color: AppColors.gold, fontSize: 42)),
          const SizedBox(height: 4),
          const Text('الوقت المتبقي',
              style: TextStyle(fontFamily: 'NotoNaskhArabic', color: Colors.white60, fontSize: 13)),
        ],
      ),
    );
  }
  ...
  Widget _buildPrayersList(PrayerTimesData data) {
    final prayerColors = {'fajr': AppColors.fajr, 'dhuhr': AppColors.dhuhr, 'asr': AppColors.asr, 'maghrib': AppColors.maghrib, 'isha': AppColors.isha};
    return Column(
      children: data.todayPrayers.entries.map((entry) {
        ...
        return Container(
          ...
          decoration: BoxDecoration(
            color: isNext ? color.withOpacity(0.12) : Theme.of(context).cardTheme.color,
            borderRadius: BorderRadius.circular(14),
            border: isNext ? Border.all(color: color.withOpacity(0.4), width: 1.5) : Border.all(color: AppColors.divider),
          ),
          child: ListTile(
            ...
            title: Text(ArabicUtils.prayerName(entry.key),
                style: AppTypography.heading3.copyWith(color: isNext ? color : AppColors.textPrimary),
                textDirection: TextDirection.rtl),
            leading: Text(ArabicUtils.toArabicTime(entry.value),
                style: TextStyle(fontFamily: 'NotoNaskhArabic', fontSize: 18,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text('يتطلب هذا الإذن الوصول إلى موقعك لحساب أوقات الصلاة',
                style: AppTypography.body, textAlign: TextAlign.center, textDirection: TextDirection.rtl),
            const SizedBox(height: 24),
            ElevatedButton(onPressed: () => ref.invalidate(prayerTimesProvider), child: const Text('إعادة المحاولة')),
          ],
        ),
      ),
    );
      ...
      builder: (_) => Directionality(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text('طريقة حساب أوقات الصلاة', style: AppTypography.heading3),
              const SizedBox(height: 16),
              ...AppConstants.calculationMethods.map((m) => ListTile(
                    title: Text(m['name'] as String, style: AppTypography.body, textDirection: TextDirection.rtl),
